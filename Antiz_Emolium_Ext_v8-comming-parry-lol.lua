...
local RotationBox    = Tabs.Main:AddRightGroupbox("Rotation Test")
...
-- ============================================================
-- Rotation Test (test different rotation methods)
-- ============================================================
RotationBox:AddToggle("RotTestEnable", {
    Text = "Enable Rotation Test", Default = false
})
RotationBox:AddDropdown("RotTestMode", {
    Values = {
        "Flat Position (fromMatrix)",
        "Flatten Y (lookAt)",
        "LookVector (fromMatrix Y-only)",
        "atan2 (CFrame.Angles)",
        "Lerp Y fromMatrix",
        "CFrame.fromMatrix (pure)",
        "BodyGyro",
        "AlignOrientation",
    },
    Default = 1, Text = "Rotation Method"
})
RotationBox:AddToggle("RotTestHorizVel", {
    Text = "Horizontal Velocity Only (prediction)", Default = true
})
RotationBox:AddSlider("RotTestLerpAlpha", {
    Text = "Lerp Alpha", Default = 100, Min = 1, Max = 100, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. "%" end
})
RotationBox:AddSlider("RotTestGyroP", {
    Text = "BodyGyro Power (P)", Default = 100000, Min = 1000, Max = 1000000, Rounding = 0,
    FormatDisplayValue = function(_, v) return tostring(v) end
})
RotationBox:AddSlider("RotTestGyroD", {
    Text = "BodyGyro Damping (D)", Default = 100, Min = 0, Max = 1000, Rounding = 0,
    FormatDisplayValue = function(_, v) return tostring(v) end
})
RotationBox:AddSlider("RotTestAlignResp", {
    Text = "AlignOrient Responsiveness", Default = 200, Min = 1, Max = 1000, Rounding = 0,
    FormatDisplayValue = function(_, v) return tostring(v) end
})
RotationBox:AddSlider("RotTestMaxTorque", {
    Text = "Max Torque (Y axis)", Default = 9, Min = 1, Max = 12, Rounding = 0,
    FormatDisplayValue = function(_, v) return "1e" .. v end
})
...
Toggles.RotTestEnable:OnChanged(function()
    if not Toggles.RotTestEnable.Value then
        if rotTestCleanup then rotTestCleanup() end
    end
end)

Options.RotTestMode:OnChanged(function()
    -- destroy physics instances when method changes
    if rotTestCleanup then rotTestCleanup() end
end)
...
-- ============================================================
-- Rotation Test state + dispatcher
-- ============================================================
local rotTestGyro    = nil
local rotTestAlign   = nil
local rotTestAtt0    = nil
local rotTestAtt1    = nil
local rotTestLastHRP = nil

function rotTestCleanup()
    if rotTestGyro    then pcall(function() rotTestGyro:Destroy()    end) rotTestGyro    = nil end
    if rotTestAlign   then pcall(function() rotTestAlign:Destroy()   end) rotTestAlign   = nil end
    if rotTestAtt0    then pcall(function() rotTestAtt0:Destroy()    end) rotTestAtt0    = nil end
    if rotTestAtt1    then pcall(function() rotTestAtt1:Destroy()    end) rotTestAtt1    = nil end
    rotTestLastHRP = nil
end

local function ensureGyro(hrp, maxTorqueY, p, d)
    if rotTestAlign then pcall(function() rotTestAlign:Destroy() end) rotTestAlign = nil end
    if rotTestAtt0  then pcall(function() rotTestAtt0:Destroy()  end) rotTestAtt0  = nil end
    if rotTestAtt1  then pcall(function() rotTestAtt1:Destroy()  end) rotTestAtt1  = nil end
    if rotTestGyro and rotTestGyro.Parent ~= hrp then
        pcall(function() rotTestGyro:Destroy() end); rotTestGyro = nil
    end
    if not rotTestGyro then
        rotTestGyro = Instance.new("BodyGyro")
        rotTestGyro.Name = "RotTestGyro"
        rotTestGyro.Parent = hrp
    end
    rotTestGyro.MaxTorque = Vector3.new(0, maxTorqueY, 0)
    rotTestGyro.P = p
    rotTestGyro.D = d
    return rotTestGyro
end

local function ensureAlign(hrp, maxTorque, responsiveness)
    if rotTestGyro then pcall(function() rotTestGyro:Destroy() end) rotTestGyro = nil end
    if rotTestAlign and rotTestAlign.Parent ~= hrp then
        pcall(function() rotTestAlign:Destroy() end); rotTestAlign = nil
        if rotTestAtt0 then pcall(function() rotTestAtt0:Destroy() end) rotTestAtt0 = nil end
        if rotTestAtt1 then pcall(function() rotTestAtt1:Destroy() end) rotTestAtt1 = nil end
    end
    if not rotTestAtt0 or rotTestAtt0.Parent ~= hrp then
        if rotTestAtt0 then pcall(function() rotTestAtt0:Destroy() end) end
        rotTestAtt0 = Instance.new("Attachment")
        rotTestAtt0.Name = "RotTestAtt0"
        rotTestAtt0.Parent = hrp
    end
    if not rotTestAtt1 then
        rotTestAtt1 = Instance.new("Attachment")
        rotTestAtt1.Name = "RotTestAtt1"
        rotTestAtt1.Parent = workspace.Terrain
    end
    if not rotTestAlign then
        rotTestAlign = Instance.new("AlignOrientation")
        rotTestAlign.Name = "RotTestAlign"
        rotTestAlign.Mode = Enum.OrientationAlignmentMode.OneAttachment
        rotTestAlign.Attachment0 = rotTestAtt0
        rotTestAlign.Parent = hrp
    end
    rotTestAlign.MaxTorque = maxTorque
    rotTestAlign.Responsiveness = responsiveness
    return rotTestAlign
end

local function applyRotationTest(hrp, myPos, predicted)
    local mode  = Options.RotTestMode.Value
    local alpha = bound((Options.RotTestLerpAlpha.Value or 100) / 100, 0.01, 1)
    local maxT  = 10 ^ (Options.RotTestMaxTorque.Value or 9)

    local rawDir = Vector3.new(predicted.X - myPos.X, 0, predicted.Z - myPos.Z)
    if rawDir.Magnitude < 0.01 then return end

    -- destroy physics instances when not using them
    if mode ~= "BodyGyro" and rotTestGyro then
        pcall(function() rotTestGyro:Destroy() end); rotTestGyro = nil
    end
    if mode ~= "AlignOrientation" and rotTestAlign then
        pcall(function() rotTestAlign:Destroy() end); rotTestAlign = nil
        if rotTestAtt0 then pcall(function() rotTestAtt0:Destroy() end) rotTestAtt0 = nil end
        if rotTestAtt1 then pcall(function() rotTestAtt1:Destroy() end) rotTestAtt1 = nil end
    end

    if mode == "Flat Position (fromMatrix)" then
        -- CFrame.fromMatrix + flat Y + Lerp (best for TSB per docs)
        local flat = Vector3.new(predicted.X, myPos.Y, predicted.Z)
        local forward = (flat - myPos)
        if forward.Magnitude < 0.01 then return end
        forward = forward.Unit
        local right = Vector3.new(0,1,0):Cross(forward).Unit
        local target = CFrame.fromMatrix(myPos, right, Vector3.new(0,1,0), -forward)
        hrp.CFrame = hrp.CFrame:Lerp(target, alpha)

    elseif mode == "Flatten Y (lookAt)" then
        -- target Y replaced by my Y, classic lookAt
        local flatTarget = Vector3.new(predicted.X, myPos.Y, predicted.Z)
        local target = CFrame.lookAt(myPos, flatTarget)
        hrp.CFrame = hrp.CFrame:Lerp(target, alpha)

    elseif mode == "LookVector (fromMatrix Y-only)" then
        -- yaw only via fromMatrix, world Y up, X/Z dir
        local dir = rawDir.Unit
        local right = dir:Cross(Vector3.new(0,1,0)) * -1
        local target = CFrame.fromMatrix(myPos, right, Vector3.new(0,1,0), -dir)
        hrp.CFrame = hrp.CFrame:Lerp(target, alpha)

    elseif mode == "atan2 (CFrame.Angles)" then
        -- pure yaw via math.atan2
        local dx = predicted.X - myPos.X
        local dz = predicted.Z - myPos.Z
        local angle = math.atan2(-dz, dx) - math.pi/2
        local target = CFrame.new(myPos) * CFrame.Angles(0, angle, 0)
        hrp.CFrame = hrp.CFrame:Lerp(target, alpha)

    elseif mode == "Lerp Y fromMatrix" then
        -- Same as Flat Position but explicitly lerp-driven smooth
        local flat = Vector3.new(predicted.X, myPos.Y, predicted.Z)
        local forward = (flat - myPos).Unit
        local right = Vector3.new(0,1,0):Cross(forward).Unit
        local newUp = forward:Cross(right).Unit
        local target = CFrame.fromMatrix(myPos, right, newUp, -forward)
        hrp.CFrame = hrp.CFrame:Lerp(target, alpha)

    elseif mode == "CFrame.fromMatrix (pure)" then
        -- raw fromMatrix using actual 3D target (no Y flatten)
        local forward = (predicted - myPos)
        if forward.Magnitude < 0.01 then return end
        forward = forward.Unit
    ...
    elseif mode == "BodyGyro" then
        local p = Options.RotTestGyroP.Value
        local d = Options.RotTestGyroD.Value
        local g = ensureGyro(hrp, maxT, p, d)
        local flat = Vector3.new(predicted.X, myPos.Y, predicted.Z)
        g.CFrame = CFrame.lookAt(myPos, flat)

    elseif mode == "AlignOrientation" then
        local resp = Options.RotTestAlignResp.Value
        local a = ensureAlign(hrp, maxT, resp)
        local flat = Vector3.new(predicted.X, myPos.Y, predicted.Z)
        a.CFrame = CFrame.lookAt(myPos, flat)
    end
end
...
local function updateAim()
    ...
    -- Rotation Test: horizontal-only velocity for prediction (yaw aim)
    if Toggles.RotTestEnable.Value and Toggles.RotTestHorizVel.Value then
        velForPred = Vector3.new(velForPred.X, 0, velForPred.Z)
    end
    ...
    -- ========== Rotation Test override ==========
    if Toggles.RotTestEnable.Value then
        applyRotationTest(HumanoidRootPart, myPos, predicted)
        return
    end
