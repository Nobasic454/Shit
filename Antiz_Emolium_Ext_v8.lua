...
SilentBox:AddDropdown("AimMode", {
    Values = {"DT", "DOT", "VectorLerp", "PerfectLerp", "DT+Lerp", "Snap", "Lerp", "Hybrid", "Instant", "Aggressive", "Adaptive", "Step"},
    Default = 6, Text = "Aim Mode"
})
...
SilentBox:AddSlider("StepAngle", {
    Text = "Step Angle", Default = 5, Min = 1, Max = 45, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. "°" end
})
SilentBox:AddSlider("StepInterval", {
    Text = "Step Interval", Default = 0.05, Min = 0.01, Max = 0.30, Rounding = 2,
    FormatDisplayValue = function(_, v) return v .. " s" end
})
...
local function updateAim()
    ...
    -- ========== DT (delta-time exponential smooth) ==========
    if aimMode == "DT" then
        local dtSpeed = Options.AimSpeed.Value / 100 * 20
        local dt = 1 / 60
        local alpha = 1 - math.exp(-dtSpeed * dt)
        HumanoidRootPart.CFrame = currentCFrame:Lerp(targetCFrame, bound(alpha, 0, 1))

    -- ========== DOT (angle-scaled blend) ==========
    elseif aimMode == "DOT" then
        local baseSpd = Options.AimBlendFactor.Value / 100
        local dotVal  = currentLook:Dot(targetLook)
        local angleFactor = 1 - dotVal  -- 0 when aligned, up to 2 when opposite
        local alpha = bound(baseSpd * angleFactor * 2, 0, 1)
        HumanoidRootPart.CFrame = currentCFrame:Lerp(targetCFrame, alpha)

    -- ========== VECTOR LERP (lerp look target, no CFrame twist) ==========
    elseif aimMode == "VectorLerp" then
        local alpha = Options.LerpMultiplier.Value
        local currentTarget = myPos + currentLook
        local newTarget = currentTarget:Lerp(predicted, alpha)
        HumanoidRootPart.CFrame = CFrame.new(myPos, newTarget)

    -- ========== PERFECT LERP (axis-angle, shortest path, no 360 bug) ==========
    elseif aimMode == "PerfectLerp" then
        local smooth = Options.AimBlendFactor.Value / 100
        local dot2   = bound(currentLook:Dot(targetLook), -1, 1)
        local angle2 = math.acos(dot2)
        if angle2 >= 1e-4 then
            local axis2 = currentLook:Cross(targetLook)
            if axis2.Magnitude >= 1e-4 then
                axis2 = axis2.Unit
                local newAngle = angle2 * smooth
                HumanoidRootPart.CFrame = CFrame.new(myPos)
                    * (currentCFrame - currentCFrame.Position)
                    * CFrame.fromAxisAngle(axis2, newAngle)
            else
    ...
    -- ========== DT + PERFECT LERP (best: dt alpha fed into axis-angle) ==========
    elseif aimMode == "DT+Lerp" then
        local dtSpeed = Options.AimSpeed.Value / 100 * 20
        local dt      = 1 / 60
        local smooth  = bound(1 - math.exp(-dtSpeed * dt), 0, 1)
        local dot2    = bound(currentLook:Dot(targetLook), -1, 1)
        local angle2  = math.acos(dot2)
        if angle2 >= 1e-4 then
            local axis2 = currentLook:Cross(targetLook)
    ...
    -- ========== SNAP (threshold gates snap; below threshold = hold) ==========
    elseif aimMode == "Snap" then
        ...
        -- below threshold: do nothing, hold current rotation
    ...
    -- ========== INSTANT (pure snap, no sliders involved) ==========
    elseif aimMode == "Instant" then
        HumanoidRootPart.CFrame = targetCFrame
    ...
    elseif aimMode == "Step" then
        local stepAngle    = Options.StepAngle and Options.StepAngle.Value or 5
        local stepInterval = Options.StepInterval and Options.StepInterval.Value or 0.05
        local now = tick()
        if angleDiff > 0.5 and (now - lastStepTime) >= stepInterval then
            lastStepTime = now
            local stepsNeeded = math.ceil(angleDiff / stepAngle)
            local stepBlend   = bound(1 / stepsNeeded, 0, 1)
            HumanoidRootPart.CFrame = currentCFrame:Lerp(targetCFrame, stepBlend)
        end
    end
end
