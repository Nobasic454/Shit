...
local function buildSnowButton(cfg)
    ...
    dragProgressBG.ZIndex                 = 16
    dragProgressBG.ClipsDescendants       = true
    dragProgressBG.Active                 = false
    newCorner(dragProgressBG, CORNER_RADIUS)

    local dragProgressFill = Instance.new("Frame")
    dragProgressFill.Parent                 = dragProgressBG
    dragProgressFill.Size                   = UDim2.new(1, 0, 0, 0)
    dragProgressFill.Position               = UDim2.new(0, 0, 1, 0)
    dragProgressFill.AnchorPoint            = Vector2.new(0, 1)
    dragProgressFill.BackgroundColor3       = cfg.dragFillColor or ICE.DRAG_READY
    dragProgressFill.BackgroundTransparency = 1.0
    dragProgressFill.BorderSizePixel        = 0
    dragProgressFill.ZIndex                 = 17
    dragProgressFill.Active                 = false
    newCorner(dragProgressFill, 1)
    newGrad3(dragProgressFill, cfg.dragGlow or ICE.DRAG_GLOW, ICE.WHITE, cfg.dragGlow or ICE.DRAG_GLOW, 90)
    ...
    for i = 1, 60 do
        ...
        sf.Position               = UDim2.new(rnd(0, 1.0), 0, rnd(-0.15, 1.0), 0)
        sf.Text                   = snowChars[(i % #snowChars) + 1]
        sf.TextSize               = math.random(2, 6)
        sf.Font                   = Enum.Font.Code
        sf.BackgroundTransparency = 1
        sf.TextColor3             = (i % 4 == 0) and (cfg.snowColorAlt or ICE.PURE) or (cfg.snowColorMain or ICE.FLAKE)
        sf.TextTransparency       = rnd(0.30, 0.68)
        sf.ZIndex                 = 14
        sf.Active                 = false
        snowflakes[i] = {
            label  = sf,
            speedY = rnd(0.012, 0.040),
            startX = rnd(0, 1.0),
    ...
    return {
        ...
        dragProgressBG=dragProgressBG, dragProgressFill=dragProgressFill, dragHint=dragHint,
        cfg=cfg,
    }
end
...
local AIM = buildSnowButton({
    px=0.02, py=0.60,
    idleTopColor = Color3.fromRGB(30,80,200), idleBotColor = Color3.fromRGB(5,10,50),
        ...
        while gui.Parent do
            ...
            for _, sf in ipairs(btnData.snowflakes) do
                ...
                if py > 1.15 then
                    ...
                    px = rnd(0, 1.0)
                    sf.startX = px
                    sl.TextTransparency = rnd(0.25, 0.65)
                end
...
local function hideDragReady(btnData)
    ...
    tw(btnData.dragProgressFill, TW.FAST, { Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1.0 })
end
...
local function onPressDown(btnData)
    ...
    tw(btnData.dragProgressBG, TW.SNAP, { BackgroundTransparency = 0.80 })
    tw(btnData.dragProgressFill, TW.SNAP, { BackgroundTransparency = 0.35 })
    TweenSvc:Create(btnData.dragProgressFill,
        TweenInfo.new(DRAG_THRESHOLD, Enum.EasingStyle.Linear, Enum.EasingDirection.Out),
...
local function onPressUp(btnData)
    ...
    tw(btnData.dragProgressFill, TW.FAST, { Size = UDim2.new(1, 0, 0, 0), BackgroundTransparency = 1.0 })
    tw(btnData.dragProgressBG, TW.FAST, { BackgroundTransparency = 1.0 })
...
local function setAimVisual(on)
    ...
    if on then
        ...
        tw(AIM.btnGrad, TW.FAST, { Color=ColorSequence.new{
            ...
            ColorSequenceKeypoint.new(1,Color3.fromRGB(5,10,50))} })
        ...
        tw(AIM.rootGrad, TW.MED, { Color=ColorSequence.new{
            ColorSequenceKeypoint.new(0,Color3.fromRGB(8,20,80)),
            ColorSequenceKeypoint.new(1,Color3.fromRGB(2,4,28))} })
...
local function makeDraggable(btnData)
    ...
    return function() return moved end  -- wasMoved()
end
