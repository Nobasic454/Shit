...
local function getLowestHealthPlayer()
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP and isAlive(p) then
            ...
            if hum and hum.Health < bestHP then best = p; bestHP = hum.Health end
        end
    end
    return best
end

local function getMostHealthPlayer()
    local best, bestHP = nil, -math.huge
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP and isAlive(p) then
            local hum = p.Character:FindFirstChildOfClass("Humanoid")
...
local function getRandomPlayer()
    local alive = {}
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LP and isAlive(p) then table.insert(alive, p) end
    end
...
local function getFirstTarget()
    local mode = Options.TargetMode and Options.TargetMode.Value or "Custom"
    ...
    elseif mode == "Nearest Switching" then
    ...
    elseif mode == "Random" then
        if not isAlive(lockedTarget) then
            lockedTarget = getRandomPlayer()
        end

    elseif mode == "Lowest Health" then
        if not isAlive(lockedTarget) then
            lockedTarget = getLowestHealthPlayer()
        end

    elseif mode == "Most Health" then
        if not isAlive(lockedTarget) then
            lockedTarget = getMostHealthPlayer()
        end
    end

    if lockedTarget and lockedTarget.Character then
        local root = lockedTarget.Character:FindFirstChild("HumanoidRootPart")
        if root then return lockedTarget, root end
    end
    return nil, nil
end
...
-- ─── Camera ───────────────────────────────────────────────────────────────────
-- From Target: always BindToRenderStep priority 0, reasserts CameraSubject every frame.
local cam            = workspace.CurrentCamera
local camOrigSubject = nil
local camOrigType    = nil

local function getTargetCamPart()
    local p, _ = getFirstTarget()
    if not p or not p.Character then return nil end
    local partName = Options.CamPartPriority and Options.CamPartPriority.Value or "HumanoidRootPart"
    if partName == "Humanoid" then
        return p.Character:FindFirstChildOfClass("Humanoid")
    end
    return p.Character:FindFirstChild(partName)
        or p.Character:FindFirstChild("HumanoidRootPart")
end

local function restoreCamera()
    pcall(function() RunService:UnbindFromRenderStep("_AntizCam") end)
...
local function camTick()
    if not Toggles.ContTpEnable.Value or Options.CameraMode.Value ~= "From Target" then return end
    local subject = getTargetCamPart()
    if subject and cam.CameraSubject ~= subject then
...
local function applyCamera()
    ...
    if mode == "From Target" and Toggles.ContTpEnable.Value then
        ...
        RunService:BindToRenderStep("_AntizCam", 0, camTick)
    end
end
...
local jitterDir        = 1
local jitterAngle      = 0
local pitchJitterDir   = 1
local pitchJitterAngle = 0
local rollJitterDir    = 1
local rollJitterAngle  = 0
...
local function getOffset(targetRoot, mode, dist)
    ...
    elseif mode == "Pitch Spin" then
        ...
        return (lv * math.cos(pitchAngle) + uv * math.sin(pitchAngle)) * dist
    elseif mode == "Roll Spin" then
        ...
        return (rv * math.cos(rollAngle) + uv * math.sin(rollAngle)) * dist
    elseif mode == "Yaw Jitter" then
    ...
    elseif mode == "Pitch Jitter" then
        local ss = Options.JitterStartDir.Value == "Right" and 1 or -1
        local es = Options.JitterEndDir.Value   == "Right" and 1 or -1
        local aS = math.rad(Options.JitterStart.Value * ss)
        local aE = math.rad(Options.JitterEnd.Value   * es)
        pitchJitterAngle = pitchJitterAngle + Options.SpinSpeed.Value * 0.05 * pitchJitterDir
        if pitchJitterDir > 0 and pitchJitterAngle >= 1 then pitchJitterAngle = 1; pitchJitterDir = -1
        elseif pitchJitterDir < 0 and pitchJitterAngle <= 0 then pitchJitterAngle = 0; pitchJitterDir = 1 end
        local a = aS + (aE - aS) * pitchJitterAngle
        return (lv * math.cos(a) + uv * math.sin(a)) * dist
    elseif mode == "Roll Jitter" then
        local ss = Options.JitterStartDir.Value == "Right" and 1 or -1
        local es = Options.JitterEndDir.Value   == "Right" and 1 or -1
        local aS = math.rad(Options.JitterStart.Value * ss)
        local aE = math.rad(Options.JitterEnd.Value   * es)
        rollJitterAngle = rollJitterAngle + Options.SpinSpeed.Value * 0.05 * rollJitterDir
        if rollJitterDir > 0 and rollJitterAngle >= 1 then rollJitterAngle = 1; rollJitterDir = -1
        elseif rollJitterDir < 0 and rollJitterAngle <= 0 then rollJitterAngle = 0; rollJitterDir = 1 end
        local a = aS + (aE - aS) * rollJitterAngle
        return (rv * math.cos(a) + uv * math.sin(a)) * dist
    end
...
ContBox:AddDropdown("TargetMode", {
    Values  = {"Random","Nearest","Nearest Switching","Lowest Health","Most Health","Custom"},
    Default = 6,
    Text    = "Target Mode",
})
...
ContBox:AddDropdown("TpMode", {
    Values  = {"above","back","forward","right","left","down","Yaw Spin","Pitch Spin","Roll Spin","Yaw Jitter","Pitch Jitter","Roll Jitter"},
    Default = 1, Text = "Mode",
})
...
ContBox:AddSlider("SpinSpeed",  {Text = "Spin / Jitter Speed", Default = 3, Min = 1, Max = 400, Rounding = 1})
