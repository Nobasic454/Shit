...
local function stopLock(explicitChar)
    ...
    spiralTime     = 0
    if highlightObj then highlightObj:Destroy(); highlightObj = nil end
    -- используем явно переданный чар или LocalPlayer.Character
    local char = explicitChar or LocalPlayer.Character
    if char then
        local hum = char:FindFirstChildOfClass("Humanoid")
...
local function onCharacterAdded(char)
    stopLock(char)  -- передаём новый чар явно
    clearAllNoclip()
    clearAllPlus()
    velocityEnabled = false
    velocityArmed   = false
    ...
    -- ещё раз после WaitForChild — гарантия что Humanoid уже точно есть
    local newHum = char:WaitForChild("Humanoid", 5)
    if newHum then newHum.AutoRotate = true end
