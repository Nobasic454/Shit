...
SilentBox:AddDropdown("TargetPart", {
    Values = {"Pivot","Center Mass","Bounding Box","Head","Torso","Left Arm","Right Arm","Left Leg","Right Leg","Random Limb","Auto Limb","Nearest Part","Auto","Bone Priority"},
...
NoCollPlusBox:AddDropdown("NoCollPlusParts", {
    Values = {"Head","Torso","Right Arm","Left Arm","Right Leg","Left Leg","HumanoidRootPart","Humanoid"},
    ...
    for partName, on in pairs(sel) do
        if on then
            if partName == "Humanoid" then
                ...
                if myChar and not plusHumConstraints[p] then
                    local cons = {}
                    for _, a in pairs(getParts(myChar)) do
                        for _, b in pairs(getParts(p.Character)) do
                            local nc = Instance.new("NoCollisionConstraint")
                            nc.Part0 = a; nc.Part1 = b; nc.Parent = a
                            table.insert(cons, nc)
                        end
                    end
                    plusHumConstraints[p] = cons
                end
            else
