...
SilentBox:AddDropdown("TargetPart", {
    Values = {"HumanoidRootPart","Torso","Head","Right Arm","Left Arm","Right Leg","Left Leg","Humanoid"},
    Default = 1, Text = "Target Part"
})
SilentBox:AddDropdown("AimMode", {
    Values = {"Instant", "Aggressive", "Lerp", "\194\176", "Snap", "Step", "Magnetic", "Adaptive", "Hybrid", "Linear", "Burst", "Velocity Aim", "Elastic", "Momentum", "PID", "Spiral"},
    Default = 1, Text = "Aim Mode"
})
SilentBox:AddDropdown("RotationMode", {
    Values = {"HumanoidRootPart", "Humanoid"},
    Default = 1, Text = "Rotation Mode"
})
...
SilentBox:AddSlider("MagneticRadius", {
    Text = "Sticky Radius", Default = 35, Min = 1, Max = 180, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. "\194\176" end
})
...
SilentBox:AddSlider("VelocityAimStrength", {
    Text = "Strength", Default = 70, Min = 1, Max = 100, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. "%" end
})
SilentBox:AddDivider()
SilentBox:AddLabel("— Elastic Settings —")
SilentBox:AddSlider("ElasticForce", {
    Text = "Spring Force", Default = 60, Min = 1, Max = 200, Rounding = 0,
    FormatDisplayValue = function(_, v) return tostring(v) end
})
SilentBox:AddSlider("ElasticDamping", {
    Text = "Damping", Default = 65, Min = 1, Max = 99, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. "%" end
})
SilentBox:AddDivider()
SilentBox:AddLabel("— Momentum Settings —")
SilentBox:AddSlider("MomentumAccel", {
    Text = "Acceleration", Default = 30, Min = 1, Max = 200, Rounding = 0,
    FormatDisplayValue = function(_, v) return tostring(v) end
})
SilentBox:AddSlider("MomentumFriction", {
    Text = "Friction", Default = 82, Min = 1, Max = 99, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. "%" end
})
SilentBox:AddDivider()
SilentBox:AddLabel("— PID Settings —")
SilentBox:AddSlider("PidP", {
    Text = "P (Proportional)", Default = 40, Min = 1, Max = 200, Rounding = 0,
    FormatDisplayValue = function(_, v) return tostring(v) end
})
SilentBox:AddSlider("PidI", {
    Text = "I (Integral)", Default = 5, Min = 0, Max = 50, Rounding = 0,
    FormatDisplayValue = function(_, v) return tostring(v) end
})
SilentBox:AddSlider("PidD", {
    Text = "D (Derivative)", Default = 15, Min = 0, Max = 100, Rounding = 0,
    FormatDisplayValue = function(_, v) return tostring(v) end
})
SilentBox:AddDivider()
SilentBox:AddLabel("— Spiral Settings —")
SilentBox:AddSlider("SpiralFrequency", {
    Text = "Frequency", Default = 20, Min = 1, Max = 100, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. " Hz" end
})
SilentBox:AddSlider("SpiralAmplitude", {
    Text = "Amplitude", Default = 10, Min = 1, Max = 45, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. "\194\176" end
})
SilentBox:AddDivider()
...
TrustBox:AddSlider("TrustHumanoid", {Text = "Humanoid",         Default = 0.00, Min = 0, Max = 1, Rounding = 2})
...
local function getTargetPart(p)
    ...
    if partName == "Humanoid" then
        ...
        return hum and hum.RootPart or p.Character:FindFirstChild("HumanoidRootPart")
    end
...
local function updateAim()
    ...
    local rotMode = Options.RotationMode and Options.RotationMode.Value or "HumanoidRootPart"
    local function applyRotation(newCFrame)
        if rotMode == "Humanoid" then
            local char = LocalPlayer.Character
            if char then pcall(function() char:PivotTo(newCFrame) end) end
        else
    ...
    if Toggles.TrustingEnabled.Value then
        local trustMap = {
            ...
            ["Humanoid"]  = Options.TrustHumanoid.Value,
        }
    ...
    elseif aimMode == "Magnetic" then
        ...
        if angleDiff <= radius then
            ...
            local expPull = (math.exp(proximity * 4.0) - 1) / (math.exp(4.0) - 1)
            magneticPull = strength * expPull
        else
            local ratio = radius / math.max(angleDiff, 0.01)
            magneticPull = strength * (ratio ^ 1.5) * 0.8
        end

        local distBoost = 1 + bound(1 - distance / 50, 0, 1.0)
        local dtFactor  = bound(dt * Options.TargetHz.Value, 0.1, 4)
        local magneticBlend = bound(magneticPull * distBoost * dtFactor, 0.005, 1)
        applyRotation(currentCFrame:Lerp(targetCFrame, magneticBlend))
    ...
    elseif aimMode == "Velocity Aim" then
        ...
        local velScale = bound(speed / 50, 0.05, 3)
        local blend    = bound(base * velScale, 0.01, 1)
        applyRotation(currentCFrame:Lerp(targetCFrame, blend))

    -- ========== ELASTIC ==========
    elseif aimMode == "Elastic" then
        local force   = Options.ElasticForce.Value
        local damping = Options.ElasticDamping.Value / 100
        local rawDir  = predicted - myPos
        if rawDir.Magnitude >= 0.01 then
            local tgtCF   = CFrame.lookAt(myPos, myPos + rawDir.Unit)
            local curLook = HumanoidRootPart.CFrame.LookVector
            local tgtLook = tgtCF.LookVector
            local cross   = curLook:Cross(tgtLook).Y
            local dot     = math.clamp(curLook:Dot(tgtLook), -1, 1)
            local errRad  = math.acos(dot) * (cross >= 0 and 1 or -1)
            elasticVel = elasticVel + errRad * force * dt
            elasticVel = elasticVel * (1 - damping)
            local step = elasticVel * dt
            applyRotation(HumanoidRootPart.CFrame * CFrame.Angles(0, step, 0))
        end

    -- ========== MOMENTUM ==========
    elseif aimMode == "Momentum" then
        local accel    = Options.MomentumAccel.Value
        local friction = Options.MomentumFriction.Value / 100
        local rawDir   = predicted - myPos
        if rawDir.Magnitude >= 0.01 then
            local tgtCF   = CFrame.lookAt(myPos, myPos + rawDir.Unit)
            local curLook = HumanoidRootPart.CFrame.LookVector
            ...
            local dot     = math.clamp(curLook:Dot(tgtLook), -1, 1)
            local errRad  = math.acos(dot) * (cross >= 0 and 1 or -1)
            momentumVel = momentumVel + errRad * accel * dt
            momentumVel = momentumVel * friction
            local step  = momentumVel * dt
            applyRotation(HumanoidRootPart.CFrame * CFrame.Angles(0, step, 0))
        end

    -- ========== PID ==========
    elseif aimMode == "PID" then
        local kP = Options.PidP.Value
        local kI = Options.PidI.Value
        local kD = Options.PidD.Value
        local rawDir = predicted - myPos
        if rawDir.Magnitude >= 0.01 then
            local tgtCF   = CFrame.lookAt(myPos, myPos + rawDir.Unit)
            local curLook = HumanoidRootPart.CFrame.LookVector
            ...
            local dot     = math.clamp(curLook:Dot(tgtLook), -1, 1)
            local errRad  = math.acos(dot) * (cross >= 0 and 1 or -1)
            pidIntegral   = pidIntegral + errRad * dt
            local deriv   = (errRad - pidLastError) / math.max(dt, 0.0001)
            pidLastError  = errRad
            local output  = kP * errRad + kI * pidIntegral + kD * deriv
            local step    = math.clamp(output * dt, -math.pi, math.pi)
            applyRotation(HumanoidRootPart.CFrame * CFrame.Angles(0, step, 0))
        end
