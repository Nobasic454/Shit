...
SilentBox:AddDropdown("AimMode", {
    Values = {"Instant", "Instant+", "Lerp", "Lerp+", "Lerp++"},
    Default = 3, Text = "Aim Mode"
})
SilentBox:AddSlider("AimAlpha", {
    Text = "Strength", Default = 65, Min = 1, Max = 100, Rounding = 0,
...
local function updateAim()
    ...
    if aimMode == "Instant" then
        -- Мгновенный поворот: CFrame.new(pos, lookAt)
        -- Позиционирует HRP на месте, направляя взгляд к цели
        local snapDelay = Options.SnapSpeedDelay and Options.SnapSpeedDelay.Value or 0
        local now = tick()
        if now - lastSnapTime >= snapDelay then
            HumanoidRootPart.CFrame = CFrame.new(myPos, predicted)
            lastSnapTime = now
        end

    elseif aimMode == "Instant+" then
        -- Мгновенный поворот: CFrame.lookAt(from, to)
        -- Современный явный API, идентичен Instant по результату
        local snapDelay = Options.SnapSpeedDelay and Options.SnapSpeedDelay.Value or 0
        local now = tick()
        if now - lastSnapTime >= snapDelay then
            HumanoidRootPart.CFrame = CFrame.lookAt(myPos, predicted)
            lastSnapTime = now
        end

    elseif aimMode == "Lerp" then
        -- Плавный Lerp с умным адаптивным alpha и LerpMultiplier
        -- Скорость нарастает при большом угловом рассогласовании
    ...
    elseif aimMode == "Lerp+" then
        -- Lerp без angleBoost: Strength × LerpMultiplier × AimSpeed
        -- Читает HumanoidRootPart.CFrame непосредственно в выражении
        local lerpMult  = Options.LerpMultiplier and Options.LerpMultiplier.Value or 1.0
        local alpha     = math.clamp(baseSpeed * speedMult * lerpMult, 0, 1)
        HumanoidRootPart.CFrame = HumanoidRootPart.CFrame:Lerp(targetCFrame, alpha)

    elseif aimMode == "Lerp++" then
        -- Lerp без angleBoost: Strength × LerpMultiplier × AimSpeed
        -- Явно кэширует current CFrame перед записью (безопаснее при многопоточности)
        local lerpMult  = Options.LerpMultiplier and Options.LerpMultiplier.Value or 1.0
        local alpha     = math.clamp(baseSpeed * speedMult * lerpMult, 0, 1)
        local current   = HumanoidRootPart.CFrame
        HumanoidRootPart.CFrame = current:Lerp(targetCFrame, alpha)
    end
end
