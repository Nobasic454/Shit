...
local function updateAim()
    ...
    -- только горизонтальная скорость: Y не влияет на yaw-поворот
    local velForPred = Vector3.new(vel.X, 0, vel.Z)
    if speed > 40 then velForPred = velForPred * 0.55 end

    if isLimb then
        velForPred = velForPred:Lerp(Vector3.new(lastVel.X, 0, lastVel.Z), 0.35) * 0.80
    end
    ...
    -- fromMatrix: чистый yaw, никогда не даёт pitch/наклон
    local dir = predicted - myPos
    if dir.Magnitude < 0.01 then return end
    dir = dir.Unit
    local worldUp       = Vector3.new(0, 1, 0)
    local right         = worldUp:Cross(dir).Unit
    local targetCFrame  = CFrame.fromMatrix(myPos, right, worldUp, -dir)
