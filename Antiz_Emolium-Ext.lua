...
SilentBox:AddDropdown("AimMode", {
    Values = {
        "Snap",          -- 1: snap по углу + плавный lerp
        "Lerp",          -- 2: стандартный плавный lerp
        "Hybrid",        -- 3: snap при высокой скорости/угле, lerp иначе
        "Instant",       -- 4: мгновенный поворот (с задержкой)
        "Aggressive",    -- 5: агрессивный lerp (0.85–1.0 alpha)
        "Spring",        -- 6: физическая пружина (velocity + damping)
        "Exponential",   -- 7: экспоненциальное затухание (frame-rate независимо)
        "SoftDrift",     -- 8: мягкое дрейфование (очень плавно, для passive play)
        "SmartAdaptive", -- 9: скорость lerp адаптируется к дистанции до цели
        "StepSnap",      -- 10: фиксированный угол за кадр (точный контроль угловой скорости)
    },
...
local springVelocity    = Vector3.zero  -- для Spring режима
local lastUpdateTime    = tick()        -- для Exponential (frame-rate independent)
...
local function updateAim()
    ...
    -- Базовый prediction: position + velocity * t
    if Toggles.AccelPredEnable.Value then
        local mult   = Options.PredStrength.Value / 100
        local offset = velForPred * t * mult + 0.5 * accel * (t ^ 2) * mult
    ...
    if Toggles.AggrAccelEnable.Value then
        ...
        local aggrOffset = velForPred * tAggr * speedBoost + accel * (tAggr ^ 2 * 0.6) * mult
    ...
    -- ── Delta time (frame-rate independent modes) ─────────────────────────────
    local now = tick()
    local dt  = math.clamp(now - lastUpdateTime, 0.001, 0.1)
    lastUpdateTime = now

    -- ── Aim Modes ─────────────────────────────────────────────────────────────

    if aimMode == "Instant" then
        -- Мгновенный snap. SnapSpeedDelay даёт паузу между срабатываниями.
    ...
    elseif aimMode == "Lerp" then
        -- Стандартный адаптивный lerp. LerpMultiplier тянет alpha вниз (мягче).
        -- LerpMultiplier 1.0 = полная адаптивная скорость; 0.10 = очень мягко.
    ...
    elseif aimMode == "Snap" then
        -- Snap-режим: при angleDiff > SnapThreshold — мгновенный поворот,
        -- иначе — плавный lerp. SnapSpeedDelay лимитирует частоту снапов.
    ...
    elseif aimMode == "Aggressive" then
        -- Агрессивный режим. AggressiveMultiplier 1→ ×2.5, 5→ ×6.5 итоговой alpha.
        -- Практически всегда > 0.85 — почти instant, но без резких прыжков.
    ...
    elseif aimMode == "Hybrid" then
        -- Hybrid: если цель быстро движется (HybridThreshold) ИЛИ угол > 30° —
        -- snap. В остальных случаях — плавный lerp × 1.5.
    ...
    elseif aimMode == "Spring" then
        -- Физическая пружина. Накапливает угловую скорость, damping гасит.
        -- AimSpeed → stiffness жёсткость. AggressiveMultiplier → damping.
        -- Даёт "живое", слегка инерционное движение — как в приватках.
        local stiffness = math.clamp(speedMult * 18, 4, 40)
        local damping   = math.clamp(1 - (Options.AggressiveMultiplier and Options.AggressiveMultiplier.Value or 1) * 0.08, 0.55, 0.92)
        local toTarget  = targetCFrame.LookVector - currentCFrame.LookVector
        springVelocity  = springVelocity * damping + toTarget * (stiffness * dt)
        -- Clamp угловой скорости — без этого пружина улетает при больших углах
        if springVelocity.Magnitude > 0.9 then
            springVelocity = springVelocity.Unit * 0.9
        end
        local newLook = (currentCFrame.LookVector + springVelocity)
        if newLook.Magnitude > 0.001 then
            HumanoidRootPart.CFrame = CFrame.new(myPos, myPos + newLook.Unit)
        end

    elseif aimMode == "Exponential" then
        -- Экспоненциальное затухание — frame-rate independent lerp.
        -- alpha = 1 - e^(-k * dt). LerpMultiplier управляет k (скоростью).
        -- При высоком AimSpeed быстро закрывает большой угол, потом плавно доводит.
        local lerpMult = Options.LerpMultiplier and Options.LerpMultiplier.Value or 1.0
        local k        = math.clamp(baseSpeed * speedMult * lerpMult * 12, 1, 60)
        local expAlpha = math.clamp(1 - math.exp(-k * dt), 0, 1)
        HumanoidRootPart.CFrame = currentCFrame:Lerp(targetCFrame, expAlpha)
        springVelocity = Vector3.zero

    elseif aimMode == "SoftDrift" then
        -- Мягкое дрейфование. Очень медленная, почти незаметная интерполяция.
        -- Идеально для passiv follow-up или камерного плавного наведения.
        -- AimAlpha/AimSpeed снижены коэффициентом 0.3 — всегда мягко.
        local driftAlpha = math.clamp(baseSpeed * speedMult * 0.3, 0.01, 0.45)
        HumanoidRootPart.CFrame = currentCFrame:Lerp(targetCFrame, driftAlpha)
        springVelocity = Vector3.zero

    elseif aimMode == "SmartAdaptive" then
        -- Адаптируется к дистанции: чем ближе цель — тем быстрее поворот.
        -- На дальней дистанции — мягко; на ближней — почти мгновенно.
        -- Формула: alpha × (0.4 + 1.6 × (1 - clamp(dist/100, 0, 1)))
        local maxDist    = 100
        local distFactor = 1 - math.clamp(distance / maxDist, 0, 1)
        local adaptAlpha = math.clamp(finalAlpha * (0.4 + distFactor * 1.6), 0, 1)
        HumanoidRootPart.CFrame = currentCFrame:Lerp(targetCFrame, adaptAlpha)
        springVelocity = Vector3.zero

    elseif aimMode == "StepSnap" then
        -- Фиксированный угол за кадр. Независимо от скорости поворачивается
        -- ровно на stepDeg градусов. AimSpeed → размер шага.
        -- Самый "механический" режим — идеален для точного контроля.
        local maxStep = math.clamp(speedMult * baseSpeed * 25, 0.5, 45)  -- °/frame
        if angleDiff <= 0.1 then
            HumanoidRootPart.CFrame = targetCFrame
        elseif angleDiff <= maxStep then
            HumanoidRootPart.CFrame = targetCFrame
        else
            -- Lerp на долю от нужного угла = максимальный шаг
            local stepRatio = maxStep / angleDiff
            HumanoidRootPart.CFrame = currentCFrame:Lerp(targetCFrame, math.clamp(stepRatio, 0, 1))
        end
        springVelocity = Vector3.zero
    end
end
