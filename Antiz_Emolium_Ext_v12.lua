...
PredBox:AddDropdown("PredSource", {
    Values  = {
...
PredBox:AddSlider("PredHybridBlend", {
    Text = "Hybrid Blend (Vel ↔ MoveDir)", Default = 50, Min = 0, Max = 100, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. "%" end
})
PredBox:AddSlider("PredAngularScale", {
    Text = "Angular Scale", Default = 50, Min = 0, Max = 200, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. "%" end
})
PredBox:AddSlider("PredWalkSpeedScale", {
    Text = "WalkSpeed Scale", Default = 100, Min = 0, Max = 300, Rounding = 0,
    FormatDisplayValue = function(_, v) return v .. "%" end
})
...
local function updateAim()
    ...
    local predSrc = Options.PredSource and Options.PredSource.Value or "AssemblyLinearVelocity"
    local tgtHum  = lockedTarget.Character and lockedTarget.Character:FindFirstChildOfClass("Humanoid")
    local velForPred

    if predSrc == "Velocity" then
        velForPred = root.Velocity
    elseif predSrc == "AssemblyAngularVelocity" then
        local omega = root.AssemblyAngularVelocity
        local scale = (Options.PredAngularScale and Options.PredAngularScale.Value or 50) / 100
        local rp    = root.Position
        local tangential = Vector3.new(
            omega.Y*(rootPos.Z-rp.Z) - omega.Z*(rootPos.Y-rp.Y),
            omega.Z*(rootPos.X-rp.X) - omega.X*(rootPos.Z-rp.Z),
            omega.X*(rootPos.Y-rp.Y) - omega.Y*(rootPos.X-rp.X)
        ) * scale
        velForPred = vel + tangential
    elseif predSrc == "MoveDirection" then
        local md  = tgtHum and tgtHum.MoveDirection or Vector3.zero
        local ws  = tgtHum and tgtHum.WalkSpeed or 16
        local wsc = (Options.PredWalkSpeedScale and Options.PredWalkSpeedScale.Value or 100) / 100
        velForPred = md * ws * wsc
    elseif predSrc == "WalkSpeed" then
        local ws  = tgtHum and tgtHum.WalkSpeed or 16
        local wsc = (Options.PredWalkSpeedScale and Options.PredWalkSpeedScale.Value or 100) / 100
        local dir = vel.Magnitude > 0.1 and vel.Unit or Vector3.zero
        velForPred = dir * ws * wsc
    elseif predSrc == "Hybrid" then
        local blend = (Options.PredHybridBlend and Options.PredHybridBlend.Value or 50) / 100
        local md    = tgtHum and tgtHum.MoveDirection or Vector3.zero
        local ws    = tgtHum and tgtHum.WalkSpeed or 16
        velForPred  = vel:Lerp(md * ws, blend)
    elseif predSrc == "Adaptive" then
        local inAir = tgtHum and tgtHum.FloorMaterial == Enum.Material.Air
        local md    = tgtHum and tgtHum.MoveDirection or Vector3.zero
        local ws    = tgtHum and tgtHum.WalkSpeed or 16
        velForPred  = inAir and vel or vel:Lerp(md * ws, 0.4)
    else -- AssemblyLinearVelocity (default)
        velForPred = vel
    end
